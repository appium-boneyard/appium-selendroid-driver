...
const SE_DOWNLOAD_CDNURL = process.env.npm_config_selendroid_driver_cdnurl ||
  ...
  "https://repo1.maven.org/maven2/io/selendroid/selendroid-standalone";
...
async function setupSelendroid () {
  ...
  if (await fs.exists(SE_JAR_PATH)) {
    log("Standalone jar exists, skipping download: " + SE_JAR_PATH);
  } else {
    await downloadSelendroid();
  }
  log(`Determining AndroidManifest location`);
  let manifestPath = await getFilePathFromJar(/AndroidManifest.*\.xml$/,
                                              SE_JAR_PATH);
  log(`Determining server apk location`);
  let serverPath = await getFilePathFromJar(/selendroid-server.*\.apk$/,
                                            SE_JAR_PATH);
  log(`Extracting manifest and apk to ${SE_DOWNLOAD_DIR}`);
  await exec('jar', ['xf', SE_JAR_PATH, manifestPath, serverPath], {
    cwd: SE_DOWNLOAD_DIR
  });
  log(`Copying manifest and apk to ${SE_DIR}`);
  ...
  log(`Fixing AndroidManifest icon bug`);
...
async function downloadSelendroid () {
  log(`Ensuring ${SE_DOWNLOAD_DIR} exists`);
  await fs.mkdir(SE_DIR);
  await fs.mkdir(SE_DOWNLOAD_DIR);
  log(`Downloading Selendroid standalone server version ${SE_VER} from ` +
  ...
  log(`Writing binary content to ${SE_JAR_PATH_TMP}`);
  ...
  if (fingerprint === SE_DOWNLOAD_SHA256) {
    ...
    log("Selendroid standalone server downloaded");
  } else {
