...
class SelendroidDriver extends BaseDriver {
  ...
  async createSession (caps) {
    try {
      ...
      logger.debug('About to check if app is present');
  ...
  async startSelendroidSession () {
    ...
    await this.initAUT();
    logger.debug('Finished initializing test app');
  ...
  async initAUT () {
    ...
    let {app, appPackage, fastReset, androidInstallTimeout} = this.opts;
    logger.debug(`Installing app: ${app}, ${appPackage}, ${fastReset}, ${androidInstallTimeout}`);
